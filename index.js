...
bot.on('callback_query', (ctx) => {
  ...
  if (data === 'servers') {
    ...
    db.run(
      ...
      (err) => {
        if (err) {
          ...
          ctx.reply('خطایی پیش اومد. لطفاً دوباره امتحان کن.');
        }
      }
    );
    ...
    db.run(
      ...
      (err) => {
        if (err) {
          console.error('خطا در به‌روزرسانی سفارش:', err.message);
          ctx.reply('خطایی پیش اومد. لطفاً دوباره امتحان کن.');
        }
      }
    );
...
bot.on('photo', (ctx) => {
  ...
  db.get(`SELECT * FROM orders WHERE userId = ? AND status = 'awaiting_receipt'`, [userId], (err, row) => {
    ...
    db.run(
      ...
      (err) => {
        if (err) {
          console.error('خطا در به‌روزرسانی وضعیت:', err.message);
          ctx.reply('خطایی پیش اومد. لطفاً دوباره امتحان کن.');
        }
      }
    );
    ...
    bot.telegram.sendPhoto(ADMIN_ID, fileId, {
      ...
      ctx.reply('خطایی در ارسال رسید به ادمین پیش اومد. لطفاً با پشتیبانی تماس بگیر.');
    });
